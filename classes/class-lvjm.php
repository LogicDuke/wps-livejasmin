...
final class LVJM {
	...
	public static function instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof LVJM ) ) {
			...
			// load config file.
			require_once LVJM_DIR . 'config.php';
			// load text domain.
	...
	public function load_admin_hooks() {
		register_activation_hook( LVJM_FILE, array( __CLASS__, 'activation' ) );
		register_deactivation_hook( LVJM_FILE, array( __CLASS__, 'deactivation' ) );
		register_uninstall_hook( LVJM_FILE, array( __CLASS__, 'uninstall' ) );
	}
	...
	public function get_whitelabel_id_from_url( $url ) {
		...
		$regex = '/lsl\/(?<whitelabel_id>\d+)\/css/';
		preg_match( $regex, $body, $matches, PREG_OFFSET_CAPTURE, 0 );

		if ( ! isset( $matches['whitelabel_id'] ) ) {
			return false;
		}

		return (string) $matches['whitelabel_id'][0];
	}
	...
	public function get_partner_categories() {
		...
		foreach ( $orientations as $orientation ) {
			$suffix       = 'Straight' !== $orientation ? strtolower( $orientation ) : '';
	...
	public function get_wp_cats() {
		return get_terms(
			array(
				'taxonomy'   => xbox_get_field_value( 'lvjm-options', 'custom-video-categories', 'category' ),
				'hide_empty' => false,
			)
		);
	}
	 ...
	 * @return mixed The reference of a variable.
	 */
	public function media_sideload_image( $file, $post_id, $desc = '' ) {
